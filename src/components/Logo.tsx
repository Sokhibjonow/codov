...
// Letter shapes from the codov brand kit (codov-color.svg)
...
/** The square "co" app icon */
export function LogoMark({ size = 32 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 1024 1024" aria-hidden="true">
      <rect width="1024" height="1024" rx="230" fill="#141B2D" />
      <g transform="translate(211.4,649.9) scale(0.4943)">
        <path fill="#F6F3EC" d={C} />
        <path fill="#F07A57" d={O1} />
      </g>
    </svg>
  );
}

/** "codov" wordmark; `size` is roughly the height of the old logo it replaces */
export function Logo({ size = 32, withText = true, tone = "dark", className = "" }: LogoProps) {
  ...
  return (
    <span className={`inline-flex items-center ${className}`}>
      <svg height={height} width={Math.round((height * 3086) / 788)} viewBox="8 -770 3086 788" role="img" aria-label="codov">
        <path fill={ink} d={C} />
