...
- **Техническая** (как всё устроено внутри): [`docs/`](docs/README.md) — архитектура, база данных,
  доступы, рабочее место ученика, автотесты, материалы курса, эксплуатация.
- **Для разработки**: [`HANDBOOK.md`](HANDBOOK.md) — как запустить, как пишем код, рецепты обычных задач, ловушки.
- **Для преподавателя** (на русском, вместе с материалами курса): `Documents/codov/docs` —
  руководство по админке, состояние курса START, работа с сайтом.
